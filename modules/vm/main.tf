...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-pull -i localhost, -U https://github.com/anilkumar429/roboshop-ansible.git roboshop.yml -e role_name=${local.role_name} -e app_name=${var.name} -e env=dev -e token=${var.token}"
    ]
  }
}
